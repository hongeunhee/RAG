...
def check_missing_columns(columns):
...
def parse_folder_to_dataframe(folder_path):
    ...
    if not all(col in columns for col in required_columns):
        ...
        if submitted:
            ...
            # Converting a date to a datetime format
            df['timestamp'] = df['timestamp'].apply(lambda x: parser.parse(x, fuzzy=True))
            # Sort timestamp column in descending order
...
class ChatSession:
    ...
    def send_message(self, question):
        ...
        # Process each response, there is no association between them.
        context = question
        ...
        if response.candidates and response.candidates[0].content.parts:
            ...
            raise ValueError("Response candidates or parts are empty.")
...
if __name__=="__main__":
    ...
    #model.generate_content(f"{lst}: Please analyze those payloads")
    # Output to Streamlit
